...
.flex-col { display: flex; flex-direction: column; gap: 2rem; height: 100%; }
.grid-2 { display: grid; grid-template-columns: repeat(2, 1fr); gap: 2.5rem; width: 100%; flex: 1; }
.grid-3 { display: grid; grid-template-columns: repeat(3, 1fr); gap: 2.5rem; width: 100%; flex: 1; }
.card { background: var(--surface); border: 1px solid var(--border); border-radius: 16px; padding: 2.5rem; display: flex; flex-direction: column; align-items: flex-start; box-sizing: border-box; }
.icon-wrapper { width: 48px; height: 48px; border-radius: 12px; background: var(--accent-dim); display: flex; align-items: center; justify-content: center; margin-bottom: 1.5rem; }
